...
class GCXTrustPolicyTests: XCTestCase {
    ...
    func test_swift_createManageRetrieveValidate_works() {
        ...
        let defaultPolicy = trustManager.create(type: .standard, hostName: "*.defaultHost.com")
        
        let pinCertificatePolicy = trustManager.create(type: .pinCertificate, hostName: "*.pinCertHost.com")
        
        let pinPublicKeyPolicy = trustManager.create(type: .pinPublicKey, hostName: "*.pinPubKey.com")
    
        let disabledPolicy = trustManager.create(type: .disabled, hostName: "*.disabledHost.com")
        ...
        let customPolicy = trustManager.create(type: .custom, hostName: "*.customHost.com", settings: settings)
        ...
        let removedPolicy = trustManager.removePolicy(name: "*.disabledHost.com")!
        ...
        trustManager.add(policy: removedPolicy)
        ...
        let defaultHostPolicy = trustManager.policy(for: "*.defaultHost.com")!
        ...
        let pinCertHostPolicy = trustManager.policy(for: "*.pinCertHost.com")!
        ...
        let pinPubKeyPolicy = trustManager.policy(for: "*.pinPubKey.com")!
        XCTAssert(pinPubKeyPolicy === pinPubKeyPolicy, "Objects should be equal.")
        isTrusted = pinPubKeyPolicy.validate(trust: testTrust)
        XCTAssertFalse(isTrusted, "That should FAIL in every condition as the test trust here is no valid trust object.")
        
        let disabledHostPolicy = trustManager.policy(for: "*.disabledHost.com")!
        ...
        let customHostPolicy = trustManager.policy(for: "*.customHost.com")!
